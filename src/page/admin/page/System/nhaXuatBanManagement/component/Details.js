...
import useCreateNhaXuatBan from "../hook/useCreateNhaXuatBan";
import useUpdateNhaXuatBan from "../hook/useUpdateNhaXuatBan";
...
const Details = ({
  ...
  const { mutate, isLoading: isSubmitting } = useCreateNhaXuatBan();
  ...
  const method = useForm({
    ...
    resolver: yupResolver(
      yup.object().shape({
        tenNXB: yup.string().required("Nhập tên nhà xuất bản vào đi"),
        quocGia: yup.string().required("Nhập quốc gia vào đi"),
      })
    ),
  });
