...
const TheLoaiManagement = () => {
    ...
    // Get Data
    const { theLoaiData, isDataLoading, fetchData, isFetching } = useGetDataTheLoai(
        "0",
        "0"
      );

      const { theLoaiDataDetail, isDataDetailLoading, fetchData: fetchDataDetail, isFetching: isFetchDetail } =  useGetDetailTheLoai(
        "0",
        "0",
        showSlice.initData?._id,
      );

      const {mutate: mutateDelete, isLoading: isSubmittingDelete} = useDeleteTheLoai();
    ...
    const handleDelete = async (data) => {
        await dispatch(setConfirm({
            status: 'open',
            method: async () => await mutateDelete({
                Data: {_id: data?._id},
                onSuccess: async (res) => {
    ...
    return (
        <>
            ...
            <PopupMain title={_.isEmpty(showSlice?.initData?._id) ? "Thêm Thể Loại" : "Sửa thể loại"} showSlice={showSlice} onShowSlice={onShowSlice} children={<Details data={theLoaiDataDetail} fetcher={fetchData} fetchDetail={fetchDataDetail} showSlice={showSlice} onShowSlice={onShowSlice}/>} />
            <Confirm />
        </>
    )
}
