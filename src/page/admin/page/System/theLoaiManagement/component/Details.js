...
const Details = ({ data = {}, fetchDetail, fetcher, showSlice, onShowSlice }) => {

    const { mutate, isLoading: isSubmitting } = useCreateTheLoai();

    const { mutate: mutateUpdate, isLoading: isSubmittingUpdate } = useUpdateTheLoai();
        ...
        if (showSlice?.initData?._id) {
            ...
            await mutate({
                ...
                onSuccess: async (res) => {
                    toast.success(res?.data?.message);
                    fetcher();
                    onShowSlice(prev => { return { ...prev, open: false, initData: null } })
                },
                onError: async (error) => {
                    toast.error(error?.error?.message);
                }
    ...
    return (
        <FormProvider {...method}>
            <form className="grid grid-cols-3 gap-[5px]" onSubmit={handleSubmit(submitForm)}>
                {/* Có 3 cột */}
                <div className="col-span-3">
                    <FormTextField label="Tên thể loại" name="tenTheLoai" errors={errors} required control={control} />
                </div>
                <CustomButton label="Lưu" type="submit" loading={isSubmitting || isSubmittingUpdate} />
            </form>
        </FormProvider>
    )
}
