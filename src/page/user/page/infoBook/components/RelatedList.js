...
const RelatedList = ({ data = {} }) => {
    ...
    const { sachData, isDataLoading, fetchData, isFetching } = useFindDataBook({theLoai: data?.tenTheLoai});


    const renderSlide = () => {
        return sachData?.filter((item)=> item._id != data?._id)?.map((slide, index) => {
    ...
    return (
        <div className='py-[20px] grid grid-cols-1 gap-[20px] w-full'>
            <h2 className='px-[30px] py-[10px] text-[#fff]' style={{ backgroundColor: COLOR.primaryColor }}>Danh sach lien quan</h2>
            {
                sachData?.length > 0
                    ?
                    <Swiper
