...
const MethodPayment = ({ onStep, step }) => {
  ...
  const { watch, setValue, getValues, reset, handleSubmit, formState: { errors } } = method;
  ...
  const payment = async (type) => {
    switch (type) {
      case 'momo': return await mutate({
        Data: {
          tongTien: step?.data?.thongTinGioHang?.tongGia ? step?.data?.thongTinGioHang?.tongGia : 1000
        },
        onSuccess: (res) => {
      ...
      case 'zalopay': return toast('Chưa hỗ trợ phương thức thanh toán này');
    }
  }

  return (
    <FormProvider {...method}>
      <form className="grid grid-cols-3 gap-[10px] h-full" onSubmit={handleSubmit(submitData)}>
        ...
        <div className="rounded-[10px] col-span-2 shadow-md p-[10px] h-full">
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="grid grid-cols-4 md:grid-cols-6 gap-[20px]">
              <button type="button" className="w-[60px] h-[60px] md:w-[80px] md:h-[80px] bg-cover" style={{ backgroundImage: "url('./images/momo.png')" }} onClick={() => {
                payment("momo");
              }}>
              </button>
              <button type="button" className="mt-[12px] w-[60px] h-[60px] md:w-[60px] md:h-[60px]  bg-contain" style={{ backgroundImage: "url('./images/zalopay.png')", filter: "grayscale(1)" }} onClick={() => {
                payment("zalopay")
              }}>
              </button>
            </div>
