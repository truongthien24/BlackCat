...
const BookCartItem = ({ book }) => {
    return (
        <div className="flex items-center justify-between w-full">
            <div className='flex'>
                <img src={book?.sach?.hinhAnh?.url} className="w-[50px] h-full mr-[10px]" />
                <div className="flex flex-col justify-between">
                    <h5 className="text-[14px] lg:text-[15px]" style={{ color: `${COLOR.primaryColor}` }}>{book?.sach?.tenSach}</h5>
                    <div className="flex items-center">
                        <div>
                            <p className="text-[13px]">Giá thuê: <span className="inline-block p-[5px] rounded-[5px]" style={{ backgroundColor: `${COLOR.secondaryColor}` }}>{parseInt(book?.giaThue).toLocaleString()}</span> </p>
                        </div>
                        <div className="mx-[10px]">
                            <p className="text-[13px]">Giá cọc: <span className="inline-block p-[5px] rounded-[5px]" style={{ backgroundColor: `${COLOR.secondaryColor}` }}>{parseInt(book?.sach?.gia).toLocaleString()}</span></p>
                        </div>
                        <div>
            ...
            <div className="">
                <p className="text-[12px] md:text-[14px]">Thành tiền: <span>{book?.tongTien}</span></p>
            </div>
        </div>
    )
}
...
const CheckCart = ({ onStep, step }) => {
    ...
    return (
        <div className="grid grid-cols-5 gap-[10px] w-[full]">
            ...
            <FormProvider {...method}>
                <form className="flex flex-col rounded-[10px] col-span-2 shadow-md p-[10px]">
                    <div className="w-full bg-[white] rounded-[7px] p-[7px] mb-[7px]">
                        <div className="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke={COLOR.primaryColor} className="w-6 h-6">
                    ...
                    <div>
                        <button type="button" className="w-full flex items-center justify-between bg-[white] rounded-[7px] p-[7px] mb-[7px]" onClick={handleChooseVoucher}>
                            <div className="flex items-center">
                                ...
                                <span className="ml-[7px] text-[12px] md:text-[15px]">Voucher</span>
                            </div>
                            <div className="flex items-center">
                    ...
                    <div className="grid grid-cols-1 gap-[10px]">
                        <div className="flex justify-center bg-[white] rounded-[5px] p-[10px] grid grid-cols-1 gap-[10px] text-[11px] md:text-[13px] text-[gray]">
                            <h5 className="text-[15px] text-[#000] flex items-center">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke={COLOR.secondaryColor} className="w-5 h-5 mr-[7px]">
            ...
            <ModalRules open={openRules} onOpen={onRules} title="Điều khoản của chúng tôi" />
        </div>
    )
}
